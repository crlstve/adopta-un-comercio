...
// Obtener el ID del comercio
    ...
    $commerce_slug = isset($_GET['auc_slg']) ? sanitize_text_field($_GET['auc_slg']) : null; // Corrige aquí
    $post = get_post($commerce_id);
    $post_title = $post ? $post->post_title : '';  // Valor predeterminado
    $post_slug = $post ? $post->post_name : ''; // valor del slug

    if ($commerce_id && $post_slug === $commerce_slug) {
        // Procesar el formulario
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['update_nonce_field']) && wp_verify_nonce($_POST['update_nonce_field'], 'update_status')) {
                ...
                $update_fb = isset($_POST['facebook']) ? sanitize_textarea_field($_POST['facebook']) : '';
                $update_ig = isset($_POST['instagram']) ? sanitize_textarea_field($_POST['instagram']) : '';
                // Subida de la imagen
                ...
                if ($commerce_id) {
                    ...
                    update_post_meta($commerce_id, 'facebook', $update_fb);
                    update_post_meta($commerce_id, 'instagram', $update_ig);
